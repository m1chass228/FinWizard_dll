...
QString PluginEngine::getPythonExecutable(const CachedConfig &cfg) const
{
    ...
    if (QFile::exists(venvPython)) {
        return venvPython; // Нашли venv — скрипт будет работать со своими либами!
    }

    // 2. Откат: Ищем базовый портативный или системный Python
    ...
    if (!QFile::exists(basePython)) {
        basePython = "python"; // Системный Windows
    }
#elif defined(Q_OS_MAC)
    // На Mac бинарник лежит в FinWizard.app/Contents/MacOS/
    // Портативный питон по канону кладем в Resources: FinWizard.app/Contents/Resources/python/bin/python3
    basePython = QDir(appDir).absoluteFilePath("../Resources/python/bin/python3");

    if (!QFile::exists(basePython)) {
        // На Mac ищем системный через PATH, если портативного нет
